...
class TFProcess:
    ...
    def init(self, batch_size, macrobatch=1, gpus_num=None, logbase='leelalogs'):
        ...
        planes = tf.reshape(planes, (batch_size, INPUT_CHANNELS, NUM_INTERSECTIONS))
    ...
    def construct_net(self, planes):
        ...
        x_planes = tf.reshape(planes, [-1, INPUT_CHANNELS, BOARD_SIZE, BOARD_SIZE])
        ...
        conv_pol = self.conv_block(flow, filter_size=1,
                                   ...
                                   output_channels=2,
                                   name="policy_head")
        h_conv_pol_flat = tf.reshape(conv_pol, [-1, OUTPUTS_POLICY * NUM_INTERSECTIONS])
        W_fc1 = weight_variable("w_fc_1", [OUTPUTS_POLICY * NUM_INTERSECTIONS, POTENTIAL_MOVES], self.model_dtype)
        b_fc1 = bias_variable("b_fc_1", [POTENTIAL_MOVES], self.model_dtype)
        ...
        conv_val = self.conv_block(flow, filter_size=1,
                                   ...
                                   output_channels=1,
                                   name="value_head")
        h_conv_val_flat = tf.reshape(conv_val, [-1, OUTPUTS_VALUE * NUM_INTERSECTIONS])
        W_fc2 = weight_variable("w_fc_2", [OUTPUTS_VALUE * NUM_INTERSECTIONS, LAYER_VALUE_FC_SIZE], self.model_dtype)
        b_fc2 = bias_variable("b_fc_2", [LAYER_VALUE_FC_SIZE], self.model_dtype)
        self.add_weights(W_fc2)
        self.add_weights(b_fc2)
        h_fc2 = tf.nn.relu(tf.add(tf.matmul(h_conv_val_flat, W_fc2), b_fc2))
        W_fc3 = weight_variable("w_fc_3", [LAYER_VALUE_FC_SIZE, 1], self.model_dtype)
...
class TFProcessTest(unittest.TestCase):
    def test_can_replace_weights(self):
        ...
        data.append([0.4] * 2*NUM_INTERSECTIONS * (POTENTIAL_MOVES))
        data.append([0.5] * (POTENTIAL_MOVES))
        # value
        data.extend(gen_block(1, tfprocess.residual_filters, 1))
        data.append([0.6] * NUM_INTERSECTIONS * LAYER_VALUE_FC_SIZE)
        data.append([0.7] * LAYER_VALUE_FC_SIZE)
        data.append([0.8] * LAYER_VALUE_FC_SIZE)
        data.append([0.9] * 1)
